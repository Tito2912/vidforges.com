...
function inferTypeFromSegments(segments: string[], fm: Frontmatter): PostType | undefined {
  ...
  // Blog index: /blog
  if (rest.length === 1 && rest[0] === 'blog') return 'blogIndex';

  // Blog articles: /blog/<slug> (excluding category/tag hubs)
  if (rest[0] === 'blog' && rest[1] && rest[1] !== 'category' && rest[1] !== 'tag') return 'article';

  // Legacy Akool page lives at /blog-akool
  if (rest.length === 1 && rest[0] === 'blog-akool') return 'article';
